...
Second, a candidate polynomial must have a stencil matrix $\mathbf{B}$ that is full rank.  The matrix is considered full rank if the \revthree{ratio of smallest to largest singular value is greater than \num{1e-12}}.
...
If there are multiple candidates with the same number of terms, the \revthree{ratio of minimum to maximum singular values} of $\mathbf{B}$ is calculated for each candidate, and an ordering is imposed such that the candidate with the larger \revthree{ratio} is preferred.  This ordering ensures that the preferred candidate is the highest-order polynomial with the most information content.
