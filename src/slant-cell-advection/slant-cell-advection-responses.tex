...
\end{comment}
\end{quotation}
The stencil size of the linearUpwind scheme is now mentioned in section \TODO{2.2}.  The linearUpwind stencil has about half the number of cells as the cubicFit stencil.
For a given $\ell_2$ error, the revised manuscript compares the mesh spacing and computational cost of both schemes for the horizontal transport test in section \TODO{3.1}.
...
\end{comment}
\end{quotation}
The stability constraints only remove polynomial terms for stencils near boundaries, or for stencils in regions of mesh distortions.  No polynomial terms are removed for stencils in the interior of two-dimensional or three-dimensional rectangular meshes.  The revised manuscript clarifies this on page \TODO{nnn} line \TODO{nnn}.
It is not known whether the stabilisation procedure is suitable for arbitrary three-dimensional meshes, and we have noted this in the new section \TODO{2.1.5}.
