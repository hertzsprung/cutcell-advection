...
We have presented a new multidimensional method-of-lines transport scheme, cubicFit, that \revother{enforces stability conditions} derived from a von Neumann stability analysis to make the scheme stable over steep terrain on \revone{highly-distorted, arbitrary meshes}.
The scheme has a low computational cost at runtime, requiring only $n$ multiplies per face per \revtwo{time-stage} using a stencil with $n$ cells.  Stability \revother{condition} calculations are pre-computed during model initialisation since they depend upon the mesh geometry only.
