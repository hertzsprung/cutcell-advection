...
Two analyses are performed in order to find stability \revother{conditions that are used to constrain} the weights $\mathbf{w} = \mathbf{\tilde{b}_1^+} \cdot \mathbf{m}$ as appear in equation~\eqref{eqn:weightedPinv}.  The first analysis uses a two-cell approximation to derive separate \revother{stability conditions involving} the upwind weight $w_u$ and downwind weight $w_d$.  The second analysis uses three cells to derive a \revother{stability condition that involves} all weights in a stencil.
...
\end{align}
Now, knowing that $\alpha_u + \alpha_d = 1$ (or $\alpha_d = 1 - \alpha_u$) then, using equations~\eqref{eqn:twopoint-lower} and \eqref{eqn:twopoint-upper}, \revother{we obtain the first two stability conditions,}
\begin{align}
...
We hypothesise that the three stability \revother{conditions} (equations~\ref{eqn:vn:upwind}, \ref{eqn:vn:downwind} and \ref{eqn:uuConstraint}) are necessary but not sufficient for a transport scheme on arbitrary meshes.
