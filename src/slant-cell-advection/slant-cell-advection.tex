...
\begin{abstract}
The inclusion of terrain in atmospheric models gives rise to mesh distortions and nonuniformities near the lower boundary that can degrade the accuracy of transport schemes.  In addition, low-order transport schemes are often preferred over high-order schemes because of stringent time-to-solution constraints on operational weather forecast models.
...
\begin{itemize}
	...
	\item numerical methods must be computationally efficient so that timely forecasts can be produced
	\item cubicFit uses a \TODO{new?} approximation technique that enables cheap computations at runtime \TODO{(is this true? what about OpenFOAM's existing \texttt{xxxFit} schemes?)}
...
\begin{itemize}
	...
	\item \TODO{oliviergooch-vanaltena2002}
\end{itemize}
