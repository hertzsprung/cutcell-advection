...
To calculate an approximate face value, a polynomial least squares fit is calculated from a stencil of surrounding cell centre values.  
For faces that are far away from the boundaries of a rectangular mesh, we fit the multidimensional polynomial given by equation~\eqref{eqn:fullPoly} that has nine unknown coefficients, $a_1 \ldots a_9$, using twelve cell centre values, $\phi_1 \ldots \phi_{12}$, from the upwind-biased stencil.  This yields a matrix equation
\begin{align}
	\begin{bmatrix}
