...
The time derivative is discretised using a two-stage, second-order Runge-Kutta method,
\begin{subequations}
\begin{align}
	\phi^\star &= \phi^{(n)} + \frac{\Delta t}{2} f(\phi^{(n)}) \\
	\phi^{(n+1)} &= \phi^{(n)} + \Delta t f(\phi^{\star})
\end{align}
\end{subequations}
