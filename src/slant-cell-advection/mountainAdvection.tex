...
\begin{figure}
	...
	\caption{Longest stable time-steps, $\Delta t_\mathrm{max}$, for the two-dimensional tracer transport test over a mountainous lower boundary.  Results were obtained on basic terrain-following, cut cell and slanted cell meshes at mesh spacings between $\Delta x = \SI{5000}{\meter}$ and $\Delta x = \SI{125}{\meter}$.}
	\label{fig:mountainAdvection-maxdt}
\end{figure}

A final series of tests were performed \revtwo{to determine the stability limit of cubicFit with the two-stage Heun time-stepping scheme (equation~\ref{eqn:heun}).}
...
\revtwo{For each test, the time-step was increased until the result became unstable.  The largest stable time-steps, $\Delta t_\mathrm{max}$, are presented in figure~\ref{fig:mountainAdvection-maxdt}.}
...
\TODO{somewhere in here discuss maximum Courant numbers (and include a figure?)}
