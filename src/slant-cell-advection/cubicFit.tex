...
A subset $S$ of $M(x,y)$ is ``dense'' if, whenever $x^a y^b$ are in $S$, then $x^i y^j$ is also in $S$ for all $0 \leq i \leq a$, $0 \leq j \leq b$.}
...
The candidate polynomials are all the dense subsets of $M(x,y)$ \revother{that have a cardinality greater than one and} that have a stencil matrix that is full rank.  The final stage of the cubicFit transport scheme selects a candidate polynomial and ensures that the least-squares fit is numerically stable.
...
\revtwo{To avoid numerical instabilities, a simplified, one-dimensional von Neumann analysis was performed, presented in appendix A.  The analysis is used to impose three stability constraints on the weights $\mathbf{w}$},
\begin{subequations}
\label{eqn:stability}
\begin{align}
...
A three-dimensional stencil has three times as many cells as its two-dimensional counterpart if the three-dimensional mesh has prismatic cells arranged in columns.  Hence, the computational cost during integration increases three-fold moving from two dimensions to three dimensions.}
...
\begin{multline}
	\phi = a_1 + a_2 x + a_3 y + a_4 z + a_5 x^2 + a_6 xy + a_7 y^2 + a_8 xz + a_9 yz + a_{10} z^2 + \\ a_{11} x^3 + a_{12} x^2 y + a_{13} x y^2 + a_{14} x^2 z + a_{15} x z^2 + a_{16} y z^2 + a_{17} y^2 z + a_{18} xyz \text{ .}
\end{multline}
...
\revone{The stabilisation procedure described in section~\ref{sec:stabilisation} requires further numerical experiments to verify that it is suitable for three-dimensional flows and arbitrary polyhedral meshes.  An initial three-dimensional test with uniform flow and a uniform Cartesian mesh obtained a numerically stable result, but we have not explored the three-dimensional extension of cubicFit any further.}
