...
\end{comment}
\end{quotation}
\TODO{I need to mention why I chose CFL ~= 0.4 in the manuscript.  First, this provides a `typical' time-step as recommended by Lauritzen et al. \citep{lauritzen2012}.  Second, I wanted to examine spatial errors and not time-stepping errors.
We have mentioned that stability was achieved at Courant numbers close to, but smaller than, one.  I didn't mention that Courant numbers slightly above one were unstable although I have indeed found this is the case.
  Also, I found that cubicFit was largely insensitivity to the time-step size or time-stepping method (two-stage Heun, three-stage Heun or classic RK4).}
...
\end{comment}
\end{quotation}
\TODO{Hilary?  We can't get the same accuracy with a lower degree polynomial and a smaller stencil.  Although we haven't demonstrated this here.}
...
\end{comment}
\end{quotation}
\TODO{Is this entirely true?  Hilary mentioned to me that Almut Gassmann has designed (split?) advection schemes for hexagons, for example.}
...
\TODO{SLICE-3D is a three-dimensional semi-Lagrangian finite volume scheme (conservative remapping/`cascade') that applies separate conservative remappings in each dimension.  This sounds like some form of dimension splitting, so it is not multidimensional?}
...
\end{quotation}
Reworded to say, ``
Unlike semi-Lagrangian finite volume schemes, swept-area and method-of-lines schemes \revtwo{achieve conservation for small-scale rotational flows that twist departure domains so that they are not simply-connected and self-intersect} \citep{lauritzen2011}.
''
...
\end{comment}
\end{quotation}
\TODO{I hadn't commented on triangles because we haven't included any results on triangular meshes.  I \emph{did} try the deformational flow test case on a sphere tessellated by triangles.  I found that linearUpwind was more accurate than cubicFit at finer mesh spacings.  We speculated that this was because the cubicFit stencil was unnecessarily large on triangular meshes.  Does this particular comment about equilateral triangles relate to the `opposedness' calculations performed during stencil construction?}
...
\end{comment}
\end{quotation}
\TODO{First of all, as opposed to what?  It's cheaper to reconstruct point values (face values) than it is to reconstruct cell integrals like swept-area schemes.}
...
\end{comment}
\end{quotation}
\TODO{Why, out of interest?  Because this is related to the condition number $cond(A) = \|A\| \: \|A^+\|$?  See \url{http://math.stackexchange.com/q/375421/89878}.}

\TODO{I looked into this briefly when I was implementing the scheme but couldn't find suitable literature.  This will require a code change and I'll have to rerun tests to check that the code change does not result in any changes to stencil coefficients.  I haven't thorougly tested the sensitivity to the minimum singular value threshold before.  But perhaps I should.}
...
\end{comment}
\end{quotation}
\TODO{Yes.  We restrict multipliers to be 1024 at most so that the matrix equation does not become ill-conditioned.
A previous version of cubicFit used much larger multipliers.
Maybe I can examine what happens to the condition number with various multipliers.}
...
\end{comment}
\end{quotation}
\TODO{I don't know off-hand, I'll have to check Thuburn et al. 2014 \citep{thuburn2014} and perhaps the original Heikes and Randall articles \citep{heikes-randall1995a,heikes-randall1995b}.}
...
\end{comment}
\end{quotation}
\TODO{Good question.  Using a projection means we can a two-dimensional polynomial.  Skamarock and friends have used the same projection before \citep{skamarock-menchaca2010,skamarock-gassmann2011}.  They mention that results are insensitive to the various details of the tangent-plane approximation.  I'm not sure what the details are?  They also mention that \citep{lashley2002} found little sensitivity to various approaches, too.}
