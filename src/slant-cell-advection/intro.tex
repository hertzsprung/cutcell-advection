...
Hence, the cubicFit scheme has \revthree{a computational cost that is lower than most swept-area schemes and more comparable to dimensionally-split schemes}, requiring only $n$ multiplies per cell face per \revtwo{time-stage} where $n$ is the size of the stencil.  \revtwo{Based on numerical experiments, the scheme is found to be conditionally stable up to a maximum Courant number of \TODO{2.n}.}
