...
\begin{align}
	a_1 = \mathbf{\tilde{b}_1^+} \cdot \mathbf{w} \cdot \mathbf{\phi} \label{eqn:weightedPinv}
\end{align}
...
So far, we have constructed a stencil and found a set of candidate polynomials.  Applying a weighted least squares fit to any of these candidate polynomials avoids creating an underconstrained problem.  The final stage of the advection scheme chooses a suitable candidate polynomial and appropriate weights so that the fit is numerically stable.

The approximate value of $\phi_F$ is equal to $a_1$ which is calculated from equation~\eqref{eqn:weightedPinv}.  The value of $a_1$ is equal to the dot product of $\mathbf{\tilde{b}_1^+} \cdot \mathbf{w}$ with the cell centre values $\bm{\phi}$.
If the $\bm{\phi}$ field is assumed to be smooth, 
shows that the value of $a_1$ depends on the values of the psuedo-inverse, $\mathbf{\tilde{b}_1^+}$.  If the $\phi$ field is assumed to be smooth,  A one-dimensional von Neumann analysis was performed to obtain stability constraints.is presented in the appendix
