...
\end{comment}
\end{quotation}
\TODO{I hadn't commented on triangles because we haven't included any results on triangular meshes.  Does this comment relate to the opposedness calculations?}
...
\end{comment}
\end{quotation}
\TODO{I could cite other papers that use a point wise least squares approximation.  What is their motivation for choosing it?  Would we expect weighting by cell volume to have any advantages?  It would not ensure numerical stability.  I could just mention that we haven't considered weighting by cell volume.}
...
\end{comment}
\end{quotation}
\TODO{Why, out of interest?  I looked into this briefly when I was implementing the scheme but couldn't find suitable literature.  This will require a code change and I'll have to rerun tests to check that the code change does not result in any changes to stencil coefficients.  I haven't tested the sensitivity to the minimum singular value threshold.}
...
\end{comment}
\end{quotation}
\TODO{Yes.  A previous version of cubicFit used much larger multipliers.  We restrict multipliers to be 1024 at most so that the matrix equation does not become ill-conditioned.}
...
\end{comment}
\end{quotation}
\TODO{Good question.  It means we can a two-dimensional polynomial.  \citep{skamarock-menchaca2010} mention that \citep{lashley2002} found little sensitivity to various approaches for tangent plane projections.  \citep{skamarock-gassmann2011} also found that results are insensitive to the tangent-plane approximation.}
