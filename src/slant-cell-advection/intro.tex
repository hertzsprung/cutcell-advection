...
This least-squares approach has been applied to shallow water flows \citep{cuetofelgueroso2006}, aeronautic \citep{cuetofelgueroso2007} and porous media \citep{white2017} simulations.}
...
Hence, the cubicFit scheme has \revthree{a computational cost that is lower than most swept-area schemes and more comparable to dimensionally-split schemes}, requiring only $n$ multiplies per cell face per \revtwo{time-stage} where $n$ is the size of the stencil.  \revtwo{The scheme is conditionally stable up to a maximum Courant number of \TODO{2.n}.}
